...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "compose_whatsapp") {
    ...
    $schedule_date_code = htmlspecialchars(strip_tags(isset($_REQUEST['schedule_date_code']) ? $conn->real_escape_string($_REQUEST['schedule_date_code']) : ""));
    $txt_schedule_date = htmlspecialchars(strip_tags(isset($_REQUEST['txt_schedule_date']) ? $conn->real_escape_string($_REQUEST['txt_schedule_date']) : ""));

    $slt_whatsapp_template = htmlspecialchars(strip_tags(isset($_REQUEST['template']) ? $conn->real_escape_string($_REQUEST['template']) : ""));
    $sender_mobile_nos = htmlspecialchars(strip_tags(isset($_REQUEST['sender_ids']) ? $conn->real_escape_string($_REQUEST['sender_ids']) : ""));
    ...
    if ($schedule_date_code == "NOW") {
        ...
        if ($template_name) {
            $sendto_api .= '"template_name" : "' . $template_name . '",';
        ...
        $sendto_api .= '{
            "user_id":"' . $_SESSION['yjwatsp_user_id'] . '",
             "sender_numbers": "' . $sender_mobile_nos . '",
             "group_name" : "' . $Whatsapp_group . '",
             "sms_send_time" : "' . $txt_schedule_date . '",';
        ...
        site_log_generate("Compose Whatsapp Page : User : " . $_SESSION['yjwatsp_user_name'] . "[LATER MESSAGE] api send text [$sendto_api] on " . date("Y-m-d H:i:s"), '../');
        // add bearer token
        $bearer_token = 'Authorization: ' . $_SESSION['yjwatsp_bearer_token'] . '';
        // It will call "compose_whatsapp_message" API to verify, can we access for thecompose_whatsapp_message
        $curl = curl_init();
        curl_setopt_array(
            $curl,
            array(
                CURLOPT_URL => $api_url . '/group/schedule_send_message',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
...
if (isset($_GET['getSingleTemplate_meta']) == "getSingleTemplate_meta") {
    ...
    if ($yjresponseobj->get_single_template) {
        // Initialize $stateData to store the template data
        $stateData = '';
        ...
        foreach ($yjresponseobj->get_single_template as $template) {
            ...
            // Check if $template_message_array is an array
            if (is_array($template_message_array)) {
                // Output the template message
                foreach ($template_message_array as $message_part) {
                    // print_r($message_part[0]) ;
                        // Check if the message part is of type "BODY"
                        if ($message_part[0]->type === "BODY") {
                            // Append the body text to $stateData
                            $stateData .= $message_part[0]->text;
                        }
    
                        // Output the appropriate form fields based on media type
                        if ($message_part[0]->media_type === "IMAGE") {
                            // Append HTML for image upload field to $stateData
                            $stateData .= '
                        ...
                        } if ($message_part[0]->media_type === "VIDEO") {
                            // Append HTML for video upload field to $stateData
                            $stateData .=   '<div class="form-group mb-3 row">
