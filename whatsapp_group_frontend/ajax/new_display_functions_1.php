							...
							<tr>
								...
								<td>
									...
									if ($sms->group_list[$indicator]->group_qrcode != '') {
										...
										$imagePath = $site_url . 'uploads/group_qr/' . $image;
										echo "<img src='$imagePath' style='width:100px; max-height: 100px;' onclick='openQRCodeModal(\"$imagePath\");'>";
									}
									?>
								</td>
								<td>
									Total count :
									<?= $sms->group_list[$indicator]->total_count; ?><br>
									Success count :
				...
				<table style="width: 100%;">
					...
					for ($indicator = 0; $indicator < count($contact_list); $indicator++) {
						if ($indicator % 2 == 0) { ?>
							...
							<?php } ?>
							<td>
								<input type="radio" <?php if ($indicator === 0) { ?>checked<?php } ?> class="cls_checkbox1"
									style="margin-left:15px;" id="txt_whatsapp_mobno_<?= $indicator ?>" name="txt_whatsapp_mobno"
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "get_admin_list") {
	...
	if ($data->response_status == 200) { ?>
						...
						foreach ($contact_list as $contact) {
							?>
							<?/* <li>
							   <label><input value="<?= $contact->mobile_no?>" name="txt_whatsapp_mobno[]" data-name="<?= $contact->mobile_no?>" type="checkbox"><?= $contact->mobile_no?></label>
      </li>*/?><option value="<?= $contact->mobile_no?>" data-badge=""><?= $contact->mobile_no?></option>
							<?php
						}
						?>
					<? } else if ($data->response_status == 204) {
