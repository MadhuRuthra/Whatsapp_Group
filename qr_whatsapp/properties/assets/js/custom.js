...
function checkPasswordStrength() {
	...
	if (txt_user_password.length < 8 && txt_user_password != '') {
...
function checkPasswordStrength_rc() {
	var number = /([0-9])/;
	var alphabets = /([a-zA-Z])/;
	var special_characters = /([~,!,@,#,$,%,^,&,*,-,_,+,=,?,>,<])/;
	var txt_user_password_rc = $('#txt_user_password_rc').val();
	console.log($('#txt_user_password_rc').val());
	if ($('#txt_user_password_rc').val().length < 8 && txt_user_password_rc != '') {
		console.log("Weak (should be atleast 8 characters.)");
		$('#txt_user_password_rc').css('border-color', 'red');
		return false;
	} else {
		if ($('#txt_user_password_rc').val().match(number) && $('#txt_user_password_rc').val().match(alphabets) && $('#txt_user_password_rc').val().match(special_characters)) {
			console.log("Strong");
			$('#txt_user_password_rc').css('border-color', '#a0a0a0');
			return true;
		} else if (txt_user_password_rc != '') {
			console.log("Medium (should include alphabets, numbers and special characters.)");
			$('#txt_user_password_rc').css('border-color', 'red');
			return false;
		}
