...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $call_function == "dashboard_count") {
	...
	$state1 = json_decode($response, true);
	if ($response == '') { ?>
		<script>
	...
	if ($state1['response_code'] == 1) { // If the response is success to execute this condition
		// Accessing response_code, response_status, and response_msg
		$responseCode = $state1['response_code'];
		$responseStatus = $state1['response_status'];
		$responseMsg = $state1['response_msg'];

		// Accessing total values
		$totalValues = $state1['total'];

		foreach ($totalValues as $total) {

			$userId = $total['user_id'];
			$userName = $total['user_name'];
			$totalGroups = $total['total_groups'];
			$totalActiveGroups = $total['total_active_group'];
			$totalInactiveGroups = $total['total_inactive_group'];
			$totalMember = $total['total_member'];
			$totalActiveMember = $total['total_active_member'];
			$totalAdmin = $total['total_admin'];

			if ($userName == $_SESSION['yjwatsp_user_name']) { // If the userid is equal to authenticate userid success to execute this condition
				?>
		...
		// Accessing group_wise values
		$groupWiseValues = $state1['group_wise'];
		$groupName = array();
		$totalMember = array();
		$totalActiveMember = array();
		$totalAdmin = array();
		$total_username = array();
		foreach ($groupWiseValues as $groupWise) {
			$groupMasterId = $groupWise['group_master_id'];
			$total_username[] = $groupWise['user_name'];
			$groupName[] = $groupWise['group_name'];
			$totalMember[] = $groupWise['total_member'];
			$totalActiveMember[] = $groupWise['total_active_member'];
			$totalAdmin[] = $groupWise['total_admin'];

			// Do something with the values
		} ?>
			<input type="hidden" class="form-control" name='group_name' id='group_name'
				value='<?= json_encode($groupName) ?>' />
			<input type="hidden" class="form-control" name='user_name' id='user_name'
				value='<?= json_encode($total_username) ?>' />
			<input type="hidden" class="form-control" name='total_admin' id='total_admin'
				value='<?= json_encode($totalAdmin) ?>' />
			<input type="hidden" class="form-control" name='total_member' id='total_member'
				value='<?= json_encode($totalMember) ?>' />
			<input type="hidden" class="form-control" name='total_active_member' id='total_active_member'
				value='<?= json_encode($totalActiveMember) ?>' />
						...
						for ($indicator = 0; $indicator < count($sms->sender_id); $indicator++) {
							...
							<tr>
								...
								<td>
									...
									if ($sms->sender_id[$indicator]->profile_image != '' && $sms->sender_id[$indicator]->profile_image != '-') {
								...
								<td id='id_approved_lineno_<?= $indicatori ?>'>
									<? if ($sms->sender_id[$indicator]->senderid_master_status == 'N' or $sms->sender_id[$indicator]->senderid_master_status == 'M' or $sms->sender_id[$indicator]->senderid_master_status == 'I' or $sms->sender_id[$indicator]->senderid_master_status == 'X') { ?>
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $call_function == "get_sender_id") {
	...
	if ($response == '') { ?>
			...
			<? } else if ($data->response_status == 200) { ?>
					...
					$contact_list = $data->sender_id;
					foreach ($contact_list as $contact) {
						if ($contact->senderid_master_status == 'Y') {
							?>
								<li>
									<label><input value="<?= $contact->mobile_no ?>" name="txt_whatsapp_mobno[]"
											data-name="<?= $contact->mobile_no ?>" type="checkbox">
								<?= $contact->mobile_no ?>
									</label>
								</li>
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $call_function == "get_groups") {
	// get_conatct_list Page get_conatct_list - Start
	site_log_generate("app_senderid_list List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	$sender_id = htmlspecialchars(strip_tags(isset($_GET["sender_id"]) ? $conn->real_escape_string($_GET["sender_id"]) : ""));
	...
	curl_setopt_array(
		$curl,
		array(
			CURLOPT_URL => $api_url . '/list/group_list_for_sender',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	if ($response == '') { ?>
			...
			<? } else if ($data->response_status == 200) { ?>
					<?php
					// Assuming $data->contact_list exists in your JSON structure
					$contact_list = $data->groups;
					foreach ($contact_list as $contact) {
						?>
							<li>
								<label><input value="<?= $contact->group_master_id ?>" name="txt_whatsapp_group[]"
										data-name="<?= $contact->group_name ?>" type="checkbox">
							<?= $contact->group_name ?> [
							<?= $contact->mobile_no ?> ]
								</label>
							</li>
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $call_function == "get_rights_groups") {
	// get_conatct_list Page get_conatct_list - Start
	site_log_generate("app_senderid_list List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	$sender_id = htmlspecialchars(strip_tags(isset($_GET["sender_id"]) ? $conn->real_escape_string($_GET["sender_id"]) : ""));
	...
	if ($response == '') { ?>
			...
			<? } else if ($data->response_status == 200) { ?>
					...
					// Assuming $data->contact_list exists in your JSON structure
					$contact_list = $data->groups; ?>
						<option value="" selected>Choose Whatsapp Group</option>
				<? foreach ($contact_list as $contact) {
					?>
							<option value="<?= $contact->group_master_id ?>">
						<?= $contact->group_name ?> [
						<?= $contact->mobile_no ?> ]
							</option>
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "getparliament") {
	// get_conatct_list Page get_conatct_list - Start
	site_log_generate("getparliament List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	...
	curl_setopt_array(
		$curl,
		array(
			CURLOPT_URL => $api_url . '/list/parliament_list',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	if ($response == '') { ?>
			...
			<? } else if ($data->response_status == 200) { ?>
					<?php
					// Assuming $data->contact_list exists in your JSON structure
					$contact_list = $data->parliament_list;
					foreach ($contact_list as $contact) {
						?>
							<option value="<?= $contact->parl_id ?>">
						<?= $contact->parl_name ?>
							</option>
					<?php
					}
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "getconstituency") {
	// get_conatct_list Page get_conatct_list - Start
	site_log_generate("getconstituency List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	$parliament_id = htmlspecialchars(strip_tags(isset($_GET["parliament_id"]) ? $conn->real_escape_string($_GET["parliament_id"]) : ""));
	$bearer_token = 'Authorization: ' . $_SESSION['yjwatsp_bearer_token'] . '';

	$replace_txt = '{
	    "parl_id" : "' . $parliament_id . '"
	     }';
	...
	curl_setopt_array(
		$curl,
		array(
			CURLOPT_URL => $api_url . '/list/constituency_list',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	if ($response == '') { ?>
			...
			<? } else if ($data->response_status == 200) { ?>
					<?php
					// Assuming $data->contact_list exists in your JSON structure
					$contact_list = $data->constituency_list;
					foreach ($contact_list as $contact) {
						?>
							<option value="<?= $contact->consti_id ?>">
						<?= $contact->consti_name ?>
							</option>
					<?php
					}
        ...
        "user_id" : "' . $_SESSION['yjwatsp_user_id'] . '"
					...
					if ($sms->num_of_rows > 0) {
						...
						for ($indicator = 0; $indicator < $sms->num_of_rows; $indicator++) {
							...
							$message = $sms->report[$indicator]->message;
							$media_type = $sms->report[$indicator]->media_type;
							$media_url = $sms->report[$indicator]->media_url;
							...
							<tr>
								...
								<td id='id_template_status_<?= $indicatori ?>'>
									<? if ($cmm_status == 'Y') { ?><a href="#!" class="btn btn-outline-success btn-disabled"
											style="width:90px; text-align:center">Sent</a>
								...
								<td>
									<a style="color:white;"
										onclick="message_view(`<?= $message ?>`, '<?= $media_type ?>','<?= $media_url ?>');"
			...
			<table class="table table-striped text-center" id="table-1">
				<thead>
					<tr class="text-center">
						...
						<th>Constituency</th>
						<th>Group Name</th>
						<th>Sender ID / Admin Numbers</th>
						<th>Group Link</th>
						<th>Group QR Code</th>
						<th>Group Members Details</th>
						<th>Members Count</th>
						<th>Admin Count</th>
						<th>Status</th>
						<th>Entry Date</th>
					...
					if ($sms->response_status == 200) {
						for ($indicator = 0; $indicator < count($sms->group_list); $indicator++) {
							...
							<tr>
								...
								<td>
									<?= $sms->group_list[$indicator]->zone_name ?>
								</td>
								<td>
									<?= $sms->group_list[$indicator]->parl_name ?>
								</td>
								<td>
									<?= $sms->group_list[$indicator]->consti_name ?>
								</td>
								<td>
								...
								<td>
									...
									<?php
									$admin_no = '';
									// Iterate over admin numbers
									foreach ($sms->admin_numbers as $admin) {
										// Check if group names match
										if ($sms->group_list[$indicator]->group_name == $admin->group_name) {
											$admin_no .= $admin->receiver_no . ',<br>';
										}
									}
									if ($admin_no) {
										$admin_no = rtrim($admin_no, ',');
										echo "/<br>Admin No : " . $admin_no;
									}
									?>
								</td>
								<td>
									<a href="<?= $sms->group_list[$indicator]->group_link ?>" target="_blank">
										<?= $sms->group_list[$indicator]->group_link ?>
									</a>
								</td>
								<td>
									<?php
									if ($sms->group_list[$indicator]->group_qrcode != '') {
										$image = basename($sms->group_list[$indicator]->group_qrcode);
										$imagePath = $site_url . 'uploads/group_qr/' . trim($image);
										echo "<img src='$imagePath' style='width:120px; max-height: 120px;cursor: pointer;' onclick='openQRCodeModal(\"$imagePath\");' >";
									}
									?>
								</td>
								<td>
									Success count :
									<?= $sms->group_list[$indicator]->success_count ?></br>
									Failure count :
									<?= $sms->group_list[$indicator]->failure_count ?></br>
								</td>
								<td>
									<?= $sms->group_list[$indicator]->members_count; ?>
								</td>
								<td>
									<?= $sms->group_list[$indicator]->admin_count; ?>
								</td>
								<td>
									...
									<? } elseif ($sms->group_list[$indicator]->group_master_status == 'D') { ?><a href="#!"
											class="btn btn-outline-danger btn-disabled" style="width:80px;">Deleted</a>
									<? } ?>
								</td>
								<td>
									<?= $entry_date ?>
			...
			<script>
				$('#table-1').DataTable({
					...
					columnDefs: [{
						...
						targets: [1, 2, 3, 4, 5, 6, 10, 11, 12, 13, 14]
					},
					{
						searchPanes: {
							show: false
...
// get_conatct_list Page get_conatct_list - Start
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "create_admin_list") {
	site_log_generate("app_senderid_list List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	$group_master_id = htmlspecialchars(strip_tags(isset($_GET["group_master_id"]) ? $conn->real_escape_string($_GET["group_master_id"]) : ""));
	...
	curl_setopt_array(
		$curl,
		array(
			CURLOPT_URL => $api_url . '/list/create_admin_list',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
...
// get_conatct_list Page get_conatct_list - Start
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "get_admin_list") {
	site_log_generate("app_senderid_list List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	$group_master_id = htmlspecialchars(strip_tags(isset($_GET["group_master_id"]) ? $conn->real_escape_string($_GET["group_master_id"]) : ""));
	...
	curl_setopt_array(
		$curl,
		array(
			CURLOPT_URL => $api_url . '/list/get_admin_list',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
					...
					if ($sms->response_status == 200) { // If the response is success to execute this condition
						for ($indicator = 0; $indicator < count($sms->report); $indicator++) {
							...
							if ($sms->report[$indicator]->message_is_read == 'N') {
								$tr_bg_clr = "background-color: #374b8c2b";
...
// get_conatct_list Page get_conatct_list - Start
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "getGroupDetails") {
	site_log_generate("app_senderid_list List Page : User : " . $_SESSION['yjwatsp_user_name'] . " Preview on " . date("Y-m-d H:i:s"), '../');
	$group_master_id = htmlspecialchars(strip_tags(isset($_GET["group_id"]) ? $conn->real_escape_string($_GET["group_id"]) : ""));
	...
	curl_setopt_array(
		$curl,
		array(
			CURLOPT_URL => $api_url . '/group/get_group_details',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	if ($response == '') { ?>
			...
			<? } else if ($data->response_status == 200) { ?>
					<?php
					$res_data = json_encode($data->group_data);
					echo $res_data;
					?>
			<? } else if ($data->response_status == 204) {
		echo $data->response_status;
