...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "contact_group") {
  ...
  $constituency_id = htmlspecialchars(strip_tags(isset($_REQUEST['constituency']) ? $_REQUEST['constituency'] : ""));
  ...
  if ($rsp_id == 403) {
    ...
    $json = array("status" => 1, "msg" => $response);
...
// All Rights in group_rights Page group_rights - Start
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "group_rights") {
  site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " access this page on " . date("Y-m-d H:i:s"), '../');
  // Get data
  $txt_contact_name = htmlspecialchars(strip_tags(isset($_REQUEST['contact_name_add']) ? $_REQUEST['contact_name_add'] : ""));
  $remove_mobile_numbers = htmlspecialchars(strip_tags(isset($_REQUEST['remove_mobile_numbers']) ? $_REQUEST['remove_mobile_numbers'] : ""));
  $promote_mobile_numbers = htmlspecialchars(strip_tags(isset($_REQUEST['promote_mobile_numbers']) ? $_REQUEST['promote_mobile_numbers'] : ""));
  $demote_mobile_numbers = htmlspecialchars(strip_tags(isset($_REQUEST['demote_mobile_numbers']) ? $_REQUEST['demote_mobile_numbers'] : ""));
  $demote_mobile_numbers = htmlspecialchars(strip_tags(isset($_REQUEST['demote_mobile_numbers']) ? $_REQUEST['demote_mobile_numbers'] : ""));
  $rdo_msg_setting = htmlspecialchars(strip_tags(isset($_REQUEST['rdo_msg_setting']) ? $_REQUEST['rdo_msg_setting'] : ""));
  ...
  if ($_FILES['upload_contact_add']['name'] != '') {
    $path_parts = pathinfo($_FILES["upload_contact_add"]["name"]);
    $extension = $path_parts['extension'];
    $filename = $_SESSION['yjwatsp_user_id'] . "_" . $milliseconds . "." . $extension;
    /* Location */
    $location = "../uploads/group_contact/" . $filename;
    ...
    $group_docs = $full_pathurl . "uploads/group_contact/" . $filename;
    $rspns = '';
    if (move_uploaded_file($_FILES['upload_contact_add']['tmp_name'], $location)) {
      site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " whatsapp_images file moved into Folder on " . date("Y-m-d H:i:s"), '../');
    }
    $replace_txt .= '"add_participants_docs" :"' . $group_docs . '",';

  } else if ($entry_contact) {

    $replace_txt .= '"add_participants" :  "' . $entry_contact . '",';

  }

  if ($_FILES['upload_contact_remove']['name'] != '') {
    $path_parts = pathinfo($_FILES["upload_contact_remove"]["name"]);
    $extension = $path_parts['extension'];
    $filename = $_SESSION['yjwatsp_user_id'] . "_" . $milliseconds . "." . $extension;
    /* Location */
    $location = "../uploads/group_contact/" . $filename;
    ...
    $rspns = '';
    if (move_uploaded_file($_FILES['upload_contact_remove']['tmp_name'], $location)) {
      site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " whatsapp_images file moved into Folder on " . date("Y-m-d H:i:s"), '../');
    }
    $replace_txt .= '"remove_participants_docs" :"' . $group_docs . '",';

  } else if ($remove_mobile_numbers) {

    $replace_txt .= '"remove_participants" :  "' . $remove_mobile_numbers . '",';
  }

  if ($promote_mobile_numbers) {

    $replace_txt .= '"promote_participants" :  "' . $promote_mobile_numbers . '",';
  }

  if ($demote_mobile_numbers) {

    $replace_txt .= '"demote_participants" :  "' . $demote_mobile_numbers . '",';
  }

  if ($rdo_msg_setting) {

    $replace_txt .= '"message_setting" :  "' . $rdo_msg_setting . '",';
  }

  $replace_txt .= '"group_id" : "' . $group_id . '",
                   "request_id":"' . $request_id . '"';
  ...
  curl_setopt_array(
    $curl,
    array(
      CURLOPT_URL => $api_url . '/group/admin_setting', // Create a New Group
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
  ...
  site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " api request [$replace_txt] on " . date("Y-m-d H:i:s"), '../');
  $response = curl_exec($curl);
  curl_close($curl);
  $respobj = json_decode($response);

  site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " api response [$response] on " . date("Y-m-d H:i:s"), '../');
  $rsp_id = $respobj->response_status;
  if ($rsp_id == 403 || $response == '') { ?>
    <script>
      window.location = "index";
    </script>
  <? // $json = array("status" => 2, "msg" => "Invalid User, Kindly try with valid User!!");
    ...
    $json = array("status" => 0, "msg" => "Failure: " . $respobj->response_msg);
    site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " [Failure: $respobj->response_msg] on " . date("Y-m-d H:i:s"), '../');
  } elseif ($rsp_id == 200) {
    $json = array("status" => 1, "msg" => $response);
    site_log_generate("Group Rights Page : User : " . $_SESSION['yjwatsp_user_name'] . " [Success] on " . date("Y-m-d H:i:s"), '../');
  }
}
// All Rights in group_rights Page group_rights - End
...
if ($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "delete_plan") {
  ...
  <? } elseif ($header->response_status == 200) {
    $json = array("status" => 1, "msg" => "Success");
  } else {
    $json = array("status" => 0, "msg" => "Failed. " . $header->response_msg);
  }
