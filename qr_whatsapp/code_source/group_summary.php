...
<body>
        ...
        <section class="section">
          ...
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item active"><a href="create_group">Create Group</a></div>
          ...
          <div class="row">
            <div class="col-12">
              <h4 class="text-right"><a href="create_group" class="btn btn-success"><i class="fas fa-plus"></i>
                  Create Group</a></h4>
            </div>
          </div>
  ...
  <div class="modal" tabindex="-1" role="dialog" id="admin_only_send_msg">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-body">
          <p class="msg_content" style="margin-left:30px;margin-top:20px;">If you enable setting, only admin can send
            mesasge to the group.</p>

          <p class="msg_content" style="margin-top:20px;text-align:center">Are you sure you want to Enable the setting ?
          </p>
        </div>
        <div class="modal-footer">
          <span class="error_display" id='admin_only_send_msg_error'></span>
          <button type="button" class="btn btn-success admin_only_send_msg_btn button" data-dismiss="model">Yes</button>
  ...
  <div class="modal" tabindex="-1" role="dialog" id="user_send_msg">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-body">
          <p class="msg_content" style="margin-left:30px;margin-top:20px;">If you enable setting, Members can send
            mesasge to the group.</p>
        ...
        <div class="modal-footer">
          <span class="error_display" id='user_send_msg_error'></span>
          <button type="button" class="btn btn-success user_send_msg_btn button" data-dismiss="model">Yes</button>
          <button type="button" class="btn btn-secondary button" data-dismiss="modal">Cancel</button>
        </div>
      </div>
    </div>
  ...
  <script type="text/javascript" defer>
    ...
    function admin_only_send_msg() {
      console.log("fdenfjs")
      $('#admin_only_send_msg').modal({ show: true });
      // $(".test").html(response);
    }
    function user_send_msg() {
      console.log("fdenfjs")
      $('#user_send_msg').modal({ show: true });
      // $(".test").html(response);
    }
    ...
    function user_send_msg_popup(group_master_id, indicatori, group_name, sender_no, selected_user_id, test) {
    ...
    $('#user_send_msg').find('.btn-success').on('click', function () {
      var formData = new FormData();
      console.log(indicatoris, group_names, sender_nos, selected_usr_id);
      var send_code = "&group_name=" + group_names + "&select_user_id=" + selected_usr_id + "&sender_no=" + sender_nos;
      // Add other data to formData
      formData.append('send_code', send_code);
      console.log(send_code)
      console.log(formData)

      console.log("cbsdhbcdscs")
      $.ajax({
        type: 'post',
        url: "ajax/message_call_functions.php?tmpl_call_function=user_send_msg",
        data: formData,
        contentType: false,
        processData: false,
        dataType: 'json',
        beforeSend: function () {
          ...
          $('.loading').show();
          $('.user_send_msg_btn').attr('disabled', true);
        },
        complete: function () {
    ...
    function admin_only_send_msg_popup(group_master_id, indicatori, group_name, sender_no, selected_user_id, test) {
      indicatoris = indicatori;
      group_names = group_name;
      sender_nos = sender_no;
      selected_usr_id = selected_user_id;
    ...
    $('#admin_only_send_msg').find('.btn-success').on('click', function () {
      var formData = new FormData();
      console.log(indicatoris, group_names, sender_nos, selected_usr_id);
      var send_code = "&group_name=" + group_names + "&select_user_id=" + selected_usr_id + "&sender_no=" + sender_nos;
      // Add other data to formData
      ...
      $.ajax({
        type: 'post',
        url: "ajax/message_call_functions.php?tmpl_call_function=admin_only_send_msg",
        data: formData,
        contentType: false,
        processData: false,
        dataType: 'json',
        beforeSend: function () {
          $(".loading").css('display', 'block');
          $('.loading').show();
          $('.admin_only_send_msg_btn').attr('disabled', true);
        },
        complete: function () {
          $(".loading").css('display', 'none');
          $('.loading').hide();
