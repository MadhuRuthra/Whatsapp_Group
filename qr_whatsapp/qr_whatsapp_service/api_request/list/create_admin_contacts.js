...
async function create_Admin_Contact(req) {
    ...
    try {
        ...
        var contact_list = `SELECT * FROM group_contacts where group_master_id = "${group_master_id}" and group_contacts_status = "Y" and (admin_status != 'Y' OR admin_status IS NULL) order by group_contacts_id desc`;
        ...
        logger_all.info("[create_Admin_Contact report] Failed - " + err);
