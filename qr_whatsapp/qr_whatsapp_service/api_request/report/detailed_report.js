...
async function detailed_report(req) {
  ...
  try {
    ...
    const select_campaign_query = `SELECT usr.user_name,wht.mobile_no,grp.group_name,con.campaign_name,con.mobile_no, con.mobile_id,CASE
    WHEN con.group_contacts_status = 'Y' THEN 'Success'
    ELSE 'Failure' END AS grp_con_status,con.group_contacts_status,con.comments ,DATE_FORMAT(con.group_contacts_entry_date,'%d-%m-%Y %H:%i:%s') group_contacts_entry_date FROM group_contacts con
    ...
    // let select_campaign = campaign_name? `${select_campaign_query}AND con.campaign_name = '${campaign_name}' ` :`${select_campaign_query} `
    let select_campaign_quer2 = campaign_name? `${select_campaign_query}AND con.campaign_name = '${campaign_name}' ` :`${select_campaign_query}`

    let select_campaign = user_name? `${select_campaign_quer2}AND usr.user_name = '${user_name}' ORDER BY con.group_contacts_entry_date DESC` :`${select_campaign_quer2}ORDER BY con.group_contacts_entry_date DESC`
