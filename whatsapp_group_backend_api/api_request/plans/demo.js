  ...
  try {
    ...
    if (get_plans_result.length > 0) {
      ...
      if (checkuserplans_result.length > 0) {
        ...
        if (checkplans_result.length > 0) {
          ...
          if (renewdatestartObj >= isoDateObj) {
            ...
            if (get_plan_status_result.length > 0) {
              var user_plan = `UPDATE user_plans SET payment_status = 'N' WHERE user_plans_id = ' ${user_plans_id}' `;
              logger_all.info("[select query request] : " + user_plan);
              const Rppayment_usrsmscrd_id = await db.query(user_plan);
              logger_all.info("[select query response] : " + JSON.stringify(Rppayment_usrsmscrd_id));

              var payment_log = `UPDATE payment_history_log SET payment_status = 'N' WHERE user_plans_id = ' ${user_plans_id}' `;
              logger_all.info("[select query request] : " + payment_log);
              const Rppayment_usrs_id = await db.query(payment_log);
              logger_all.info("[select query response] : " + JSON.stringify(Rppayment_usrs_id));

              var update_plans = `UPDATE plans_update SET plan_status = 'N' WHERE user_id = ' ${slt_user_id}' and plan_master_id = '${plan_master_id}'`;
              logger_all.info("[select query request] : " + update_plans);
              const update_plans_result = await db.query(update_plans);
              logger_all.info("[select query response] : " + JSON.stringify(update_plans_result));

              // var user_plan = `UPDATE user_plans SET plan_amount = '${plan_amount}', plan_reference_id = '${plan_reference_id}',payment_status = 'W', plan_comments = '${plan_comments}',user_plans_status = 'W',plan_expiry_date = '0000-00-00 00:00:00',user_plans_entdate = CURRENT_TIMESTAMP  WHERE user_id = '${slt_user_id}' and plan_master_id = '${plan_master_id}' `;

              // logger_all.info("[select query request] : " + user_plan);
              // const Rppayment_usrid = await db.query(user_plan);
              // logger_all.info("[select query response] : " + JSON.stringify(Rppayment_usrid));

              const plans_update = `INSERT INTO plans_update VALUES(NULL,'${plan_master_id}','${slt_user_id}','${whatsapp_no_max_count}','0','0','${group_no_max_count}','0','0','${message_limit}','0','0','N',CURRENT_TIMESTAMP,NULL)`;
              logger_all.info("[insert query request] : " + plans_update);
              const plans_update_result = await db.query(plans_update);
              logger_all.info("[insert query response] : " + JSON.stringify(plans_update_result));

              const insert_payment_history = `INSERT INTO payment_history_log VALUES(NULL,'${slt_user_id}','${plan_master_id}','${user_plans_id}','${plan_amount}','W','${plan_comments}','Y',CURRENT_TIMESTAMP)`;
              logger_all.info("[insert query request] : " + insert_payment_history);
              const insert_payment_history_result = await db.query(insert_payment_history);
              logger_all.info("[insert query response] : " + JSON.stringify(insert_payment_history_result));
              if (insert_payment_history_result.insertId) {
                return {
                  response_code: 1,
                  response_status: 200,
          ...
          if (get_history_result.length > 0) {
            ...
            // var user_plan = `UPDATE user_plans SET plan_amount = '${plan_amount}',plan_master_id = '${plan_master_id}', plan_reference_id = '${plan_reference_id}',payment_status = 'W', plan_comments = '${plan_comments}',user_plans_status = 'W',plan_expiry_date = '0000-00-00 00:00:00',user_plans_entdate = CURRENT_TIMESTAMP WHERE user_id = '${slt_user_id}' `;

            // logger_all.info("[select query request] : " + user_plan);
            // const Rppayment_usr_id = await db.query(user_plan);
            // logger_all.info("[select query response] : " + JSON.stringify(Rppayment_usr_id));
            ...
            const insert_payment_history = `INSERT INTO payment_history_log VALUES(NULL,'${slt_user_id}','${user_plans_id}','${plan_master_id}','${plan_amount}','W','${plan_comments}','Y',CURRENT_TIMESTAMP)`;
            logger_all.info("[insert query request] : " + insert_payment_history);
            const insert_payment_history_result = await db.query(insert_payment_history);
            logger_all.info("[insert query response] : " + JSON.stringify(insert_payment_history_result));
            if (insert_payment_history_result.insertId) {
        ...
        const insert_user_plans = `INSERT INTO user_plans VALUES(NULL,'${slt_user_id}','${plan_master_id}','${plan_amount}','0000-00-00 00:00:00','W','${plan_comments}','${plan_reference_id}','W',CURRENT_TIMESTAMP)`;
        logger_all.info("[insert query request] : " + insert_user_plans);
        const insert_user_plans_result = await db.query(insert_user_plans);
        logger_all.info("[insert query response] : " + JSON.stringify(insert_user_plans_result))

        if (!insert_user_plans_result.insertId) {
          return {
            response_code: 0,
            response_status: 201,
          ...
          const insert_payment_history = `INSERT INTO payment_history_log VALUES(NULL,'${slt_user_id}','${insert_user_plans_result.insertId}','${plan_master_id}','${plan_amount}','W','${plan_comments}','Y',CURRENT_TIMESTAMP)`;
          logger_all.info("[insert query request] : " + insert_payment_history);
          const insert_payment_history_result = await db.query(insert_payment_history);
          logger_all.info("[insert query response] : " + JSON.stringify(insert_payment_history_result));
          if (insert_payment_history_result.insertId) {
