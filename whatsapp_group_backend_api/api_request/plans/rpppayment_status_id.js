...
async function Rppayment_status_id(req) {
        var logger_all = main.logger_all
    var logger = main.logger
        try {

                // get all the req data
                var slt_user_plans_id = req.body.slt_user_plans_id;
...
// get_available_message to execute this query
                        logger_all.info("[select query request] : " + `SELECT * FROM user_plans where user_plans_id = '${slt_user_plans_id}'
            `);
                        const Rppayment_usrsmscrd_id = await db.query(`SELECT * FROM user_plans where user_plans_id = '${slt_user_plans_id}'
            `);
