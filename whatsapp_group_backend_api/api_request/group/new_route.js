                    ...
                    client.getChats().then(async (chats) => {
                        try {
                            ...
                            if (grp_id == 0) {
                                ...
                                const insert_grp = `INSERT INTO group_master VALUES(NULL,'${user_id}','${sender_master_id}','${myGroupName}','${myGroup.participants.length}','0','${myGroup.participants.length}','N','Y',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`
                        ...
                        try {
                            ...
                            if (myGroup) {
                                ...
                                if (grp_id == 0) {
                                    ...
                                    const insert_grp = `INSERT INTO group_master VALUES(NULL,'${user_id}','${sender_master_id}','${myGroupName}','${participants.length}','${contact_id.length}','${participants.length - contact_id.length}','N','Y',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`
                                    logger_all.info("[insert query request] : " + insert_grp);
                                    const insert_grp_result = await db.query(insert_grp);
                                    logger_all.info("[insert query response] : " + JSON.stringify(insert_grp_result))
                                    ...
                                    const update_grp_count = `UPDATE group_master SET total_count = total_count+${participants.length},success_count= success_count+${contact_id.length}, failure_count= failure_count+${participants.length - contact_id.length},group_updated_date = CURRENT_TIMESTAMP WHERE group_master_id = '${grp_id}' AND group_master_status = 'Y'`
                                ...
                                for (var k = 0; k < participants.length; k++) {
                                    ...
                                    const update_number_status = `INSERT INTO group_contacts VALUES(NULL,'${user_id}','${grp_id}','${campaign_name}','${participants[k]}','${participants[k]}','${response_msg[k]}','${contact_status}',CURRENT_TIMESTAMP,NULL,NULL)`
                ...
                async function create_grp() {
                    ...
                    try {
                        ...
                        if (myGroup) {
                            ...
                            const group_code = await myGroup.getInviteCode();
                            const group_link = `https://chat.whatsapp.com/${group_code}`;

                            logger_all.info(group_code, group_link + "******************");
                            console.log(group_link + "sjdjsbdj");
                            // Generate QR code
                            qr.toFile('/opt/lampp/htdocs/whatsapp_group_newapi/uploads/group_qr/' + myGroupName + '.png', group_link, (err) => {
                                if (err) throw err;

                            });
                            // Update the database with the path of the generated QR code image
                            const update_group = `UPDATE group_master SET group_link = '${group_link}', group_qrcode = '/opt/lampp/htdocs/whatsapp_group_newapi/uploads/group_qr/${myGroupName}.png', group_updated_date = CURRENT_TIMESTAMP WHERE group_name = '${myGroupName}' and group_master_status = 'Y'`;
                            logger_all.info("[update query request] : " + update_group);
                            const updateResult = await db.query(update_group);
                            logger_all.info("[update query response] : " + JSON.stringify(updateResult));
                        ...
                        const insert_grp = `INSERT INTO group_master VALUES(NULL,'${user_id}','${sender_master_id}','${myGroupName}','${participants.length}','${contact_id.length}','${participants.length - contact_id.length}','Y','Y',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,NULL,NULL)`
                        logger_all.info("[insert query request] : " + insert_grp);
                        const insert_grp_result = await db.query(insert_grp);
                        logger_all.info("[insert query response] : " + JSON.stringify(insert_grp_result))
                        ...
                        for (var k = 0; k < participants.length; k++) {
                            ...
                            const update_number_status = `INSERT INTO group_contacts VALUES(NULL,'${user_id}','${insert_grp_result.insertId}','${campaign_name}','${participants[k]}','${participants[k]}','${response_msg[k]}','${contact_status}',CURRENT_TIMESTAMP,NULL,NULL)`
                            logger_all.info("[insert query request] : " + update_number_status);
                            const update_number_status_result = await db.query(update_number_status);
                            logger_all.info("[insert query response] : " + JSON.stringify(update_number_status_result))
                    ...
                    client.getChats().then(async (chats) => {
                        try {
                            ...
                            if (grp_id == 0) {
                                ...
                                const update_grp_count = `UPDATE group_master SET total_count = total_count-${participants.length},success_count= success_count-${participants.length},group_updated_date = CURRENT_TIMESTAMP WHERE group_master_id = '${grp_id}' AND group_master_status = 'Y'`
                                logger_all.info("[update query request] : " + update_grp_count);
                                const update_grp_count_result = await db.query(update_grp_count);
                                logger_all.info("[update query response] : " + JSON.stringify(update_grp_count_result))
                                grp_id = insert_grp_result.insertId;
                ...
                async function promote_admin() {
                    ...
                    client.getChats().then(async (chats) => {
                        try {
                            ...
                            const numberDetails = await client.getNumberId(participants);
                            logger_all.info("[Phone Number Details] : " + numberDetails)
                            var admin_add = await myGroup.demoteParticipants([numberDetails._serialized]);

                            logger_all.info("[Promote admin response] : " + admin_add)
                            if (admin_add.status == 200) {

                                logger_all.info("[Promote admin response : Admin Promoted successfully.]");
                                ...
                                logger_all.info("[Promote admin response Error:]" + admin_add);
                            ...
                            const group_contacts = `UPDATE group_contacts SET admin_status = '${res_status}' WHERE group_master_id = '${grp_id}' AND mobile_no = '${participants}'`
                ...
                async function demote_admin() {
                    ...
                    client.getChats().then(async (chats) => {
                        try {
                            ...
                            logger_all.info("[Demote admin response] : " + admin_add)
                            if (admin_add.status == 200) {

                                logger_all.info("[Demote admin response : Admin demoted successfully.]");
                                ...
                                logger_all.info("[Demote admin response Error:]" + admin_add);
