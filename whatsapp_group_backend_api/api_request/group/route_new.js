...
router.post(
    ...
    async function (req, res, next) {
        try {
            ...
            console.log("Description:", group.description || "No description provided");
            console.log("Created At:", group.createdAt.toString());
            console.log("Participant count:", group.participants.length);
            console.log("Group ID:", group.id);
            console.log("Group Owner:", group.owner ? group.owner.user : "Unknown");
            console.log("Group Participants:");

            group.participants.forEach(participant => {
                console.log("Name:", participant.name);
                console.log("Phone Number:", participant.id.user);
                console.log("Role:", participant.isAdmin ? "Admin" : "Member");
            });
            console.log("-------------------------------------------------------------");
        });
            ...
            if (select_sender_result.length == 0) {
                ...
                async function create_grp() {
                    ...
                    try {
                        ...
                        const insert_grp = `INSERT INTO group_master VALUES(NULL,'${user_id}','${sender_master_id}','${myGroupName}','${participants.length}','${contact_id.length}','${participants.length - contact_id.length}','Y','Y',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`
                        logger_all.info("[insert query request] : " + insert_grp);
                        const insert_grp_result = await db.query(insert_grp);
                        logger_all.info("[insert query response] : " + JSON.stringify(insert_grp_result))
                ...
                async function create_admin() {
                    ...
                    client.getChats().then(async (chats) => {
                        try {
                           ...
                           var promote_response =  await myGroup.promoteParticipants(['919965014814']);
                           logger_all.info(JSON.stringify(promote_response));
