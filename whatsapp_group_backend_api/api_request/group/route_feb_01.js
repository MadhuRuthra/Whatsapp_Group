...
// Example date strings to convert to cron expressions
   var scheduledTimes = [];
        ...
        try {
            ...
            if (select_sender_id_status.length == 0) {
                ...
                data.forEach(item => {
                    ...
                    console.log("Text:", message_content);
                });
                ...
                async function send_message() {
                    ...
                    client.getChats().then(async (chats) => {
                        try {
                            ...
                            logger_all.info(" Group name - " + message_content)
                            ...
                            let res_msg, res_code, res_code_status, res_status;

                            const insert_msg = `INSERT INTO whatsapp_group_newapi_${user_id}.compose_message_${user_id} VALUES(NULL,'${user_id}','${sender_master_id}','${grp_id}',${template_master_id},'TEXT','${campaign_name}','N',NULL,'N',CURRENT_TIMESTAMP)`
                            ...
                            const insert_msg_content = `INSERT INTO whatsapp_group_newapi_${user_id}.compose_msg_media_${user_id} VALUES(NULL,'${insert_msg_result.insertId}','${message_content}',NULL,NULL,NULL,NULL,NULL,NULL,NULL,'N',CURRENT_TIMESTAMP)`
                            ...
                            try {
                                console.log('Group ID:', myGroup.id._serialized + 'message:', message_content);
                            ...
                            const update_msg_content = `UPDATE whatsapp_group_newapi_${user_id}.compose_msg_media_${user_id} SET cmm_status = '${res_status}' WHERE compose_msg_media_id = '${insert_msg_content_result.insertId}' AND cmm_status = 'N'`
                            logger_all.info("[update query request] : " + update_msg_content);
                            const update_msg_content_result = await db.query(update_msg_content);
                            logger_all.info("[update query response] : " + JSON.stringify(update_msg_content_result))
...
router.post(
    ...
    async function (req, res, next) {
        try {
            ...
            var response_json;
            var message_content;
            var replace_msg,remove_schedule_time;
            scheduledTimes.push(sms_send_time);
            ...
            const insert_msg = `INSERT INTO whatsapp_group_newapi_${user_id}.compose_message_${user_id} VALUES(NULL,'${user_id}','${sender_master_id}','${grp_id}',${template_master_id},'TEXT','${campaign_name}','Y','${sms_send_time}','N',CURRENT_TIMESTAMP)`
            logger_all.info("[insert query request] : " + insert_msg);
            const insert_msg_result = await db.query(insert_msg);
            logger_all.info("[insert query response] : " + JSON.stringify(insert_msg_result))
            ...
            res_code_status = 1
             ...
             logger_all.info(scheduledTimes +"scheduledTimes")
                ...
                // ARRANGE THE ORDER in schedule time
               scheduledTimes.sort((a, b) => new Date(b) - new Date(a));

              // Schedule tasks for each date string
              ...
              const cronExpression = dateStringToCronExpression(scheduledTime);
                        ...
                        client.getChats().then(async (chats) => {
                            try {
                                ...
                                try {
                                    ...
                                    res_code = 200
                                    res_code_status = 1
                              logger_all.info(" scheduledTime send " + scheduledTime)

                               remove_schedule_time = scheduledTime;
                                scheduledTimes = scheduledTimes.filter(item => item !== remove_schedule_time);
                               logger_all.info(" Schedule message arrays - " + scheduledTimes)
           ...
           /* } else {   // IF Schedule is passed using on else

                              const update_api_log = `UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'Scheduled time '${scheduledTime}' has already passed.' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`
                                logger_all.info("[update query request] : " + update_api_log);
                                const update_api_log_result = db.query(update_api_log);
                                logger_all.info("[update query response] : " + JSON.stringify(update_api_log_result))

                                response_json = { request_id: req.body.request_id, response_code: 0, response_status: 201, response_msg: 'Scheduled time ' + scheduledTime +' has already passed..' }
                                logger.info("[API RESPONSE] " + JSON.stringify(response_json))
                                logger_all.info("[API RESPONSE] " + JSON.stringify(response_json))
