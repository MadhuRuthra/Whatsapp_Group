    ...
    async function (req, res, next) {
        try {
            ...
            if (select_sender_id_status.length == 0) {
                ...
                // Create a single instance of the Client with the LocalAuth strategy
                ...
                client.on('ready', async () => {
                    ...
                    // Function to periodically check connection state and keep the client active
                    const keepClientActive = async () => {
                        try {
                    ...
                    // Periodically check connection state every 4 hours
                    setInterval(keepClientActive, 4 * 60 * 60 * 1000);
                });
                ...
                async function send_message() {
                    ...
                    try {
                        ...
                        try {
                            ...
                            const message_response = await myGroup.sendMessage(message);
                            ...
                            if (message_response.status === 'error') {
                                logger_all.info('Message could not be sent:', message_response.error);
                                console.error('Message could not be sent:', message_response.error);
                            }
