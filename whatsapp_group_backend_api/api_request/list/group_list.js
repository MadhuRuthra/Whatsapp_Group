...
async function group_list(req) {
  ...
  try {
    ...
    const group_list = `SELECT grp.group_master_id,usr.user_name,usr.user_id,con.mobile_no,grp.group_name,grp.total_count,grp.success_count,grp.failure_count,grp.group_master_status,DATE_FORMAT(grp.group_master_entdate,'%d-%m-%Y %H:%i:%s') group_master_entdate,DATE_FORMAT(grp.group_updated_date,'%d-%m-%Y %H:%i:%s') group_updated_date,grp.group_link,grp.group_qrcode,grp.members_count,grp.admin_count,grp.is_admin_only_msg FROM group_master grp LEFT JOIN senderid_master con ON con.sender_master_id = grp.sender_master_id LEFT JOIN user_management usr ON usr.user_id = con.user_id where (con.user_id = '${user_id}' or usr.parent_id = '${user_id}')
    and senderid_master_status = 'Y' ORDER BY grp.group_master_id DESC`
    ...
    LEFT JOIN group_contacts congrp ON congrp.group_master_id = grp.group_master_id where congrp.admin_status = 'Y'
...
async function group_list_Sender(req) {
  ...
  try {
    ...
    LEFT JOIN user_management usr ON usr.user_id = con.user_id where (con.user_id = '${user_id}' or usr.parent_id = '${user_id}') AND con.mobile_no in (${sender_id}) and grp.group_master_status = 'Y' and senderid_master_status = 'Y' ORDER BY grp.group_master_id DESC;`
    logger_all.info("[Select query request] : " + group_list);
    var group_list_result = await db.query(group_list);
