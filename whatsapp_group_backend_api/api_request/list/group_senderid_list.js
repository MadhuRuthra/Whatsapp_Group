...
async function SenderGroup_list(req) {
  var logger_all = main.logger_all

  let user_id = req.body.user_id;
  let group_master_id = req.body.group_master_id;

  try {

    const group_list = `SELECT grp.group_master_id,usr.user_name,usr.user_id,con.mobile_no,grp.group_name,grp.total_count,grp.success_count,grp.failure_count,grp.group_master_status,DATE_FORMAT(grp.group_master_entdate,'%d-%m-%Y %H:%i:%s') group_master_entdate FROM group_master grp
    ...
    where (con.user_id = '${user_id}' or usr.parent_id = '${user_id}') and group_master_id = ${group_master_id} GROUP by (con.mobile_no) ORDER BY grp.group_master_id DESC`
