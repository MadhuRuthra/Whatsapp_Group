...
async function Get_Group_Contact(req) {
    ...
    try {
        ...
        logger_all.info("[Select query request] : " + `SELECT * FROM group_contacts where group_master_id = "${group_master_id}" and group_contacts_status = "Y" order by group_contacts_id desc`);
        var contact_list = await db.query(`SELECT * FROM group_contacts where group_master_id = "${group_master_id}" and group_contacts_status = "Y" order by group_contacts_id desc`);
