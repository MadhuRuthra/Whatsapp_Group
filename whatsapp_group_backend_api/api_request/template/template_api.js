...
async function getTemplate(req) {
  try {
    ...
    if (user_master_id == 1) {
      get_template = `SELECT * FROM template_master where template_status = 'Y' ORDER BY template_entry_date ASC`;
    } else {
      get_template = `SELECT * FROM template_master where template_status = 'Y' and user_id = '${user_id}' ORDER BY template_entry_date ASC`
    }
