            ...
            if (get_number_result.length == 0) {
                ...
                client.on('qr', async (qr) => {
                    ...
                    if (get_number_status_result.length == 0) {
                        ...
                        console.log("**********")
                        console.log(get_number_status_result[0].senderid_master_status)
                        console.log(get_number_status_result[0].sender_master_id)
                ...
                client.on('ready', async () => {
                    ...
                    if (`${qr_number}` == `${sender_id}`) {
                        ...
                        const update_active_process = `UPDATE senderid_master SET senderid_master_status = '${status_user}' WHERE mobile_no = '${sender_id}' AND senderid_master_status != 'D'`
                        ...
                        for (var i = 0; i < chats.length; i++) {
                            ...
                            if (chats[i].isGroup) {
                                ...
                                WHERE sender_master_id = '${sender_id_master}' AND group_name = '${chats[i].name}' and group_master_status = 'Y'`
                                ...
                                if (get_frp_result.length == 0) {
                                    ...
                                    try {
                                        var myGroup = await client.getChatById(chats[i].id._serialized)
                                        console.log(myGroup);
                                        console.log("***********8")
                                        if (myGroup) {
                                            ...
                                            const insert_grp = `INSERT INTO group_master VALUES(NULL,'${user_id}','${sender_id_master}','${chats[i].name}','${chats[i].participants.length}','${chats[i].participants.length}','0','Y','Y','${formattedDateTime}',CURRENT_TIMESTAMP,'${group_link}','${media_storage}/uploads/group_qr/${current_date}.png',1,'${chats[i].participants.length}','N')`
                                            ...
                                            for (var k = 0; k < chats[i].participants.length; k++) {
                                                ...
                                                const get_members = `SELECT * FROM group_contacts where mobile_no = '${participant}' AND group_master_id = '${insert_grp_result.insertId}'`;
                                                ...
                                                if (participant.role === 'admin' || participant.permissions.includes('admin') || participant.isAdmin === true) {
                                    ...
                                    const update_grp = `UPDATE group_master SET members_count = '${chats[i].participants.length}' , success_count = '${chats[i].participants.length}' WHERE sender_master_id = '${sender_id_master}' AND group_name = '${chats[i].name}'`
                                    ...
                                    for (var k = 0; k < chats[i].participants.length; k++) {
                                        ...
                                        const get_members = `SELECT * FROM group_contacts where mobile_no = '${participant}' AND group_master_id = '${get_frp_result[0].group_master_id}'`;
                                        logger_all.info("[update query request] : " + get_members);
                                        const get_members_result = await db.query(get_members);
                                        logger_all.info("[update query response] : " + JSON.stringify(get_members_result));
                                    ...
                                    setTimeout(async function () {
                                        if (function_call == false) {
                                            ...
                                            if (fs.existsSync(`./session_copy/session-${sender_id}`)) {
                                                ...
                                                const update_api_log = `UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'Error Sendoccurred' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`
                                                logger_all.info("[update query request] : " + update_api_log);
                                                const update_api_log_result = await db.query(update_api_log);
                                                logger_all.info("[update query response] : " + JSON.stringify(update_api_log_result))
                                        ...
                                        client.getChats().then(async (chats) => {
                                            try {
                                                ...
                                                if (grp_id == 0) {
                                                    ...
                                                    const insert_grp = `INSERT INTO group_master VALUES(NULL,'${user_id}','${sender_master_id}','${get_group_master_result[0].group_name}','${myGroup.participants.length}','0','${myGroup.participants.length}','N','Y',CURRENT_TIMESTAMP,'N')`
                                                    logger_all.info("[insert query request] : " + insert_grp);
                                                    const insert_grp_result = await db.query(insert_grp);
                                                    logger_all.info("[insert query response] : " + JSON.stringify(insert_grp_result))
