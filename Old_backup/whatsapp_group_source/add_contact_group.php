...
<head>
  ...
  <title>Add Contacts in Group :: <?= $site_title ?></title>
...
<body>
                  ...
                  <form class="needs-validation" novalidate="" id="frm_contact_group" name="frm_contact_group"
                    ...
                    <div class="card-body">
                          ...
                          <table style="width: 100%;">
                            ...
                            if ($state1->response_status == 200) {
                              for ($indicator = 0; $indicator < count($state1->sender_id); $indicator++) {
                                if($state1->sender_id[$indicator]->whatspp_config_status == 'Y') {
                                if ($counter % 2 == 0) { ?>
                                  <tr>
                                  <? } ?>
                                  <td>
                                    <input type="radio" <? if($counter == 0 or $_REQUEST['sender'] == $state1->sender_id[$indicator]->phonecode . $state1->sender_id[$indicator]->mobile_no) { $firstid = $state1->sender_id[$indicator]->phonecode . $state1->sender_id[$indicator]->mobile_no; ?>checked<? } ?> onclick="func_change_groupname('<?= $state1->sender_id[$indicator]->phonecode . $state1->sender_id[$indicator]->mobile_no ?>')" class="cls_checkbox" id="txt_whatsapp_mobno_<?=$indicator?>"
                                      name="txt_whatsapp_mobno" tabindex="1" autofocus
                                      value="<?= $state1->sender_id[$indicator]->phonecode . $state1->sender_id[$indicator]->mobile_no ?>">
                                    <label class="form-label">
                                      <?= $state1->sender_id[$indicator]->phonecode . $state1->sender_id[$indicator]->mobile_no ?>
                                    </label>
                                  </td>
                                  <?
                                  if ($counter % 2 == 1) { ?>
                      ...
                      <div class="form-group mb-2 row">
                        ...
                        <div class="col-sm-7">
                          <input type="radio" name="rdo_newex_group" id="rdo_new_group" checked tabindex="2" value="N" onclick="func_open_newex_group('N')"> New Group&nbsp;&nbsp;&nbsp;<input type="radio" name="rdo_newex_group" id="rdo_ex_group" tabindex="3" value="E" <? if($_SERVER["QUERY_STRING"] != '') { ?> checked <? } ?>onclick="func_open_newex_group('E')"> Existing Group 
                        </div>
                        <div class="col-sm-2">
                        </div>
                      </div>
                      ...
                      <div class="form-group mb-2 row">
                        ...
                        <div class="col-sm-7">
                          <div id="id_new_groupname" <? if($_SERVER["QUERY_STRING"] != '') { ?> style="display: none" <? } else { ?> style="display: block" <? } ?>>
                            <input type="text" name="txt_group_name" id='txt_group_name' tabindex="4" class="form-control"
                            value="" <? if($_REQUEST["group"] == '') { ?> required="" <? } ?> maxlength="250" placeholder="Enter Group Name" data-toggle="tooltip" data-placement="top" title=""
                            data-original-title="Enter Group Name">
                          </div>
                          <div id="id_ex_groupname" <? if($_SERVER["QUERY_STRING"] != '') { ?> style="display: block" <? } else { ?> style="display: none" <? } ?>>
                              ...
                              if ($state1->response_status == 200) { 
                                ...
                                for ($indicator = 0; $indicator < count($state1->group_list); $indicator++) { 
                                  if($exgroup_name != $state1->group_list[$indicator]->group_name) { 
                                    ...
                                    $group_contact_id = $state1->group_list[$indicator]->group_contact_id; ?>
                                    <option value="<?=$group_name?>" <? if ($indicator == 0 or $_REQUEST["group"] == $group_contact_id) { echo "selected"; } ?>><?=$group_name?></option>
                                  <?php }
                      ...
                      <div class="form-group mb-2 row">
                        ...
                        <div class="col-sm-7">
                            <input type="text" name="txt_contact_name" id='txt_contact_name' tabindex="7" class="form-control"
                            value="" maxlength="1000" placeholder="Enter Contact Name" data-toggle="tooltip" data-placement="top" title=""
                            data-original-title="Enter Contact Name and comma separated" style="margin-bottom: 10px;">

                            <input type="file" class="form-control" name="upload_contact" id='upload_contact'
                            tabindex="7" accept="text/csv" data-toggle="tooltip" data-placement="top" data-html="true" title=""
  ...
  <script>
    ...
    function func_open_newex_group(group_available) {
      ...
      <? // if($_REQUEST['group'] == '') { ?>
        if(group_avail == 'N') {
          ...
          $('#id_new_groupname').css("display", "block");
    ...
    $(document).on("submit", "form#frm_contact_group", function (e) {
        ...
        /* txt_group_name field validation  */
        if(group_avail == 'N') {
          if (txt_group_name == "") {
